...
export default function TeacherDashboardContent() {
  ...
  const [activeView, setActiveView] = useState<"classes" | "students" | "plans">("classes");
    ...
    const fetchStudyPlans = async () => {
      try {
        ...
        if (Array.isArray(allStudentsWithPlans)) {
          ...
          const teacherStudentIds = new Set(studentsData.map(student => student.id));
          const teacherStudentsWithPlans = allStudentsWithPlans.filter(
            (student: any) => teacherStudentIds.has(student.id) && student.totalPlans > 0
          );
              ...
              <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {studentsData.length} students enrolled across {activeClasses} classes
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {studentsData.length === 0 ? (
                  <div className="text-center py-16 border-2 border-dashed border-gray-200 rounded-3xl">
                    ...
                    <p className="text-gray-900 font-semibold">No students enrolled yet</p>
                  ...
                  <div className="grid gap-4 md:gap-6">
                    {studentsData.map((student, index) => (
                      <div key={`${student.id}-${student.classId}`} className="group relative bg-gradient-to-r from-white to-gray-50/50 border border-gray-200 rounded-2xl p-6 hover:border-indigo-200 hover:shadow-lg hover:shadow-indigo-100/50 transition-all duration-200">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <div className="text-xs text-gray-500 mb-1">Student #{index + 1}</div>
              ...
              <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-2xl font-bold text-gray-900">Student Study Plans</h2>
                      <p className="text-sm text-gray-500">
                        {studyPlansData.length} active study plans • Monitor student progress
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {studyPlansData.length === 0 ? (
                  <div className="text-center py-16 border-2 border-dashed border-gray-200 rounded-3xl">
                    ...
                    <p className="text-gray-900 font-semibold">No study plans yet</p>
                    <p className="text-sm text-gray-500 max-w-xs mx-auto mt-1">
                  ...
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {studyPlansData.map((student) => {
                      const progressPercentage = student.totalTasks > 0 ? Math.round((student.completedTasks / student.totalTasks) * 100) : 0;

                      return (
                        <div key={student.id} className="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-200 rounded-2xl p-6 hover:shadow-lg transition-all duration-200">
                          ...
                          <div className="mb-4">
                            <div className="flex justify-between text-sm text-gray-600 mb-2">
                              <span>{student.completedTasks} of {student.totalTasks} tasks completed</span>
                            </div>
                          ...
                          <div className="flex items-center justify-between text-xs text-gray-500">
                            <span>{student.totalPlans} Study Plan{student.totalPlans !== 1 ? 's' : ''}</span>
                            <span className={`px-2 py-1 rounded-full ${
                              ...
                              {progressPercentage === 100 ? 'Complete' : progressPercentage > 50 ? 'In Progress' : 'Just Started'}
                            </span>
                          </div>
                        </div>
                      );
