...
export default function TeacherClassesView() {
  ...
  const [classSummary, setClassSummary] = useState({ totalStudents: 0, totalClasses: 0, totalQuestions: 0 });
  ...
  const fetchClasses = async () => {
    ...
    try {
      ...
      const normalized: ClassType[] = Array.isArray(classesData)
        ? classesData.map((clsRaw: unknown) => {
            ...
            return {
              ...
              students:
                clsItem.students ||
                clsItem.enrollments?.map((e) => e.student).filter((s): s is StudentType => !!s) ||
                [],
            };
          })
        : [];
      ...
      const totalStudents = normalized.reduce((sum, cls) => sum + (cls.students?.length ?? 0), 0);
      const totalQuestions = normalized.reduce((sum, cls) => sum + (cls._count?.questions ?? 0), 0);
      setClassSummary({ totalStudents, totalClasses: normalized.length, totalQuestions });
    } catch (err) {
      console.error(err);
      setClasses([]);
      setStudents([]);
  ...
  const showStudentProgress = async (student: StudentType) => {
    ...
    try {
      const res = await fetch(`/api/admin/study-plans/progress?studentId=${student.id}`);
      if (!res.ok) {
    ...
    <div className="space-y-8">
      <div className="space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage your assigned classes, questions and student responses.</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="px-3 py-2 bg-indigo-100 text-indigo-700 rounded-xl text-sm font-semibold">Classes: {classSummary.totalClasses}</span>
            <span className="px-3 py-2 bg-emerald-100 text-emerald-700 rounded-xl text-sm font-semibold">Students: {classSummary.totalStudents}</span>
            <span className="px-3 py-2 bg-blue-100 text-blue-700 rounded-xl text-sm font-semibold">Questions: {classSummary.totalQuestions}</span>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="rounded-2xl p-5 border border-gray-100 shadow-sm bg-white">
            <p className="text-xs uppercase tracking-widest text-gray-500">Active Classes</p>
            <p className="text-2xl font-bold text-gray-900">{classSummary.totalClasses}</p>
          </div>
          <div className="rounded-2xl p-5 border border-gray-100 shadow-sm bg-white">
            <p className="text-xs uppercase tracking-widest text-gray-500">Total Students</p>
          ...
          <div className="rounded-2xl p-5 border border-gray-100 shadow-sm bg-white">
            ...
            <p className="text-2xl font-bold text-gray-900">{classSummary.totalQuestions}</p>
          </div>
        </div>
      </div>
      ...
      {classes.length === 0 ? (
        <div className="bg-white rounded-3xl p-20 text-center shadow-sm border border-gray-100">
          ...
          <p className="text-gray-500">You will see your classes here once they are assigned to you.</p>
        </div>
      ) : (
              ...
              <div key={cls.id} className="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-xl transition-shadow duration-300">
                <div className="bg-gradient-to-r from-indigo-500 to-purple-600 p-6 text-white">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="bg-white/20 px-2 py-1 rounded-md text-xs">Students: {studentList.length}</span>
                      <span className="bg-white/20 px-2 py-1 rounded-md text-xs">Questions: {cls._count?.questions ?? 0}</span>
                    </div>
                  </div>
                </div>
      ...
      {selectedStudent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-xl bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-xl relative">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4">{selectedStudent.name} Progress</h3>
            ...
            {progressLoading ? (
              ...
              <div className="space-y-3">
                <p className="text-sm text-gray-600 dark:text-gray-300">Class: {studentProgress.className || "Unassigned"}</p>
                <p className="text-sm text-gray-600 dark:text-gray-300">Total Plans: {studentProgress.totalPlans}</p>
                <p className="text-sm text-gray-600 dark:text-gray-300">Total Tasks: {studentProgress.totalTasks}</p>
                <p className="text-sm text-gray-600 dark:text-gray-300">Completed Tasks: {studentProgress.completedTasks}</p>
                <p className="text-sm text-gray-600 dark:text-gray-300">Completion: {studentProgress.totalTasks > 0 ? `${Math.round((studentProgress.completedTasks / studentProgress.totalTasks) * 100)}%` : "0%"}</p>
              </div>
            ) : (
              <p className="text-sm text-gray-500">No progress data found for this student yet.</p>
            )}
          </div>
        </div>
      )}
