      ...
      <div className="p-6 border-b flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Monitor and manage students in your classes
          </p>
        </div>
        <button
          onClick={() => setShowAddModal(true)}
          className="bg-linear-to-r from-[#7C3AED] to-[#8B5CF6] text-white px-6 py-3 rounded-2xl font-semibold flex items-center gap-2 hover:shadow-lg transition"
        >
          + Add Student
        </button>
      </div>
            ...
            {students.map((student) => (
              <tr key={student.id} className="hover:bg-gray-50">
                <td className="py-5 font-medium flex items-center gap-3">
                  <div className="w-9 h-9 rounded-2xl bg-linear-to-br from-[#7C3AED] to-[#8B5CF6] text-white flex items-center justify-center font-bold">
                    {student.name[0]}
                  </div>
                  {student.name}
                </td>
                ...
                <td className="py-5">
                  <div className="flex items-center gap-3">
                    <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-linear-to-r from-[#7C3AED] to-[#6EE7B7]"
                ...
                <td className="py-5 text-right space-x-1">
                  <button className="p-2 text-gray-400 hover:text-[#7C3AED]">
                    <Eye size={18} />
                  </button>
                  <button className="p-2 text-gray-400 hover:text-[#7C3AED]">
                    <Pencil size={18} />
